...
from envs.make_env import make_vec_envs
#from models.factory import create_policy
from models.factory0310 import create_policy
...
def main():
    ...
    # print('tra_env_main2v2', type(train_envs))
    ...
    #print('rollouts.obs[0]',rollouts.obs[0].size())
    ...
    for j in tqdm(range(num_updates)):
        for step in range(args.num_steps):
            ...
            # print('action_main2v2',action)
            #print('train_envs.step',train_envs.step(action))
            obs, reward, done, infos,_ = train_envs.step(action)
            #print('obs_size_main2v2',obs.size()) #size:torch.Size([16, 2, 1092])
            # print('reward_main2v2',reward) #torch.Size([16, 1])
            #print('done_main2v2',done.size())
            ...
            '''for agent_id in range(2):
                ...
                masks1 = torch.tensor([[0.0] if done_ else [1.0] for done_ in done.transpose()[agent_id]], device=device)

                masks0.append(masks1)
            masks = torch.cat(masks0, dim=1)'''
        ...
        if j % args.log_interval == 0 and len(episode_rewards) > 1:
            ...
            print('value_loss2v2',value_loss)
        ...
        if args.eval_interval and len(episode_rewards) > 1 and j > 0 and j % args.eval_interval == 0:
            ...
            while len(eval_episode_rewards) < 200:
                with torch.no_grad():
                    _, action, _, eval_dist_entropy1, eval_recurrent_hidden_states = actor_critic.act(
            ...
            #print('eval_episode_rewards',eval_episode_rewards)
            #print('len_eval_episode_rewards',len(eval_episode_rewards))
            #log episode reward mean std
        ...
        np.savetxt('./results/coma_FFA/train_score_seed_{}.csv'.format(42), np.array(log_mean),
                   delimiter=";")
        np.savetxt('./results/coma_FFA/train_scorestd_seed_{}.csv'.format(42),np.array(log_std),
                   delimiter=";")
        np.savetxt('./results/coma_FFA/train_dist_entropy_seed_{}.csv'.format(42),np.array(log_dist_entropy),
                   delimiter=";")
        np.savetxt('./results/coma_FFA/train_dist_entropystd_seed_{}.csv'.format(42),np.array(log_dist_entropy_std),
                   delimiter=";")
