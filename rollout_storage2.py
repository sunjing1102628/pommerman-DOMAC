...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_shape, action_space,n_agents, recurrent_hidden_state_size):
        self.obs = torch.zeros(num_steps + 1, num_processes, n_agents, *obs_shape)
        self.recurrent_hidden_states = torch.zeros(num_steps + 1, num_processes, recurrent_hidden_state_size)
        self.rewards = torch.zeros(num_steps, num_processes, 1)
        self.value_preds = torch.zeros(num_steps + 1, num_processes, n_agents)
        self.returns = torch.zeros(num_steps + 1, num_processes, n_agents)
        self.action_log_probs = torch.zeros(num_steps, num_processes, n_agents)
        if action_space.__class__.__name__ == 'Discrete':
            self.actions = torch.zeros(num_steps, num_processes, n_agents).long()
        else:
